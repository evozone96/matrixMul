...
int matrixMultiply(int argc, char **argv, int block_size, dim3 &dimsA, dim3 &dimsB)
{
    ...
    for (int j = 0; j < nIter; j++)
    {
        if (block_size == 16)
        {
            matrixMulCUDA<16><<< grid, threads,0, streams[(2*j)%streamNum] >>>(d_C, d_A, d_B, dimsA.x, dimsB.x);
        }
        else
        {
            matrixMulCUDA<32><<< grid, threads,0, streams[(2*j)%streamNum] >>>(d_C, d_A, d_B, dimsA.x, dimsB.x);
        }
        if (block_size == 16)
        {
            matrixMulCUDA_int<16><<< grid, threads,0, streams[(2*j+1)%streamNum] >>>(d_C_double, d_A_double, d_B_double, dimsA.x, dimsB.x);
        }
        else
        {
            matrixMulCUDA_int<32><<< grid, threads,0, streams[(2*j+1)%streamNum] >>>(d_C_double, d_A_double, d_B_double, dimsA.x, dimsB.x);
        }
    }
